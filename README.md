...
- 当前版本：**v2.0.0**（开发完成待真机验收；v1.1.0 起为 Stitch 设计系统 + 更名「拉比克」）
- v2.0 新增：截图标注编辑器（7 工具 + 识图侧面板）、自绘选框 + 窗口吸附（挖孔遮罩所见即所截）、划图 OCR（Vision 本地）、翻译（Apple 离线 / 可选大模型，选中文字按 T 直译）、「智能识别」设置页
